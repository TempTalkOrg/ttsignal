...
xqc_packet_parse_short_header(xqc_connection_t *c, xqc_packet_in_t *packet_in)
{
    ...
    if (xqc_conn_check_dcid(c, &(packet->pkt_dcid)) != XQC_OK) {
        ...
        xqc_log(c->log, XQC_LOG_WARN, "|parse short header|invalid destination cid, pkt dcid: %s, conn scid: %s|",
                xqc_dcid_str(&packet->pkt_dcid, c->dcid_buf, sizeof(c->dcid_buf)), 
                xqc_scid_str(&c->scid_set.user_scid, c->scid_buf, sizeof(c->scid_buf)));
        return -XQC_EILLPKT;
    }
        ...
        if (path == NULL && packet_in->pi_path_id != XQC_UNKNOWN_PATH_ID) {
            ...
            if (path == NULL) {
                xqc_log(c->log, XQC_LOG_ERROR, "|can not find path|dcid:%s|path_id:%ui|",
                        xqc_dcid_str(&packet_in->pi_pkt.pkt_dcid, c->dcid_buf, sizeof(c->dcid_buf)), 
            ...
            if (xqc_cid_is_equal(&path->path_last_scid, &packet_in->pi_pkt.pkt_dcid) != XQC_OK) {
                ...
                xqc_log(c->log, XQC_LOG_DEBUG, "|update_path_scid|%s->%s|",
                        xqc_scid_str(&path->path_last_scid, c->scid_buf, sizeof(c->scid_buf)),
                        xqc_dcid_str(&path->path_scid, c->dcid_buf, sizeof(c->dcid_buf)));
    ...
    xqc_log(c->log, XQC_LOG_DEBUG, "|parse short header|path:%ui|pkt_dcid:%s|spin_bit:%ud|",
            packet_in->pi_path_id, 
            xqc_scid_str(&(packet->pkt_dcid), c->scid_buf, sizeof(c->scid_buf)), 
            spin_bit);
...
xqc_packet_parse_retry(xqc_connection_t *c, xqc_packet_in_t *packet_in)
{
    ...
    if (xqc_cid_is_equal(&c->original_dcid, &packet_in->pi_pkt.pkt_scid) == XQC_OK) {
        xqc_log(c->log, XQC_LOG_DEBUG, "|discard|packet SCID error|odcid:%s|scid:%s|",
            xqc_dcid_str(&c->original_dcid, c->dcid_buf, sizeof(c->dcid_buf)),
            xqc_scid_str(&packet_in->pi_pkt.pkt_scid, c->scid_buf, sizeof(c->scid_buf)));
        return -XQC_EILLPKT;
    }

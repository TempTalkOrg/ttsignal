...
#include "common/utils/vint/xqc_variable_len_int.h"
...
const char *
xqc_frame_type_2_str(
    xqc_frame_type_bit_t type_bit, 
    char* out_buf,
    xqc_int_t out_buf_size)
{
...
xqc_process_new_conn_id_frame(xqc_connection_t *conn, xqc_packet_in_t *packet_in)
{
    ...
    xqc_log(conn->log, XQC_LOG_DEBUG, "|new_conn_id|%s|sr_token:%s",
            xqc_scid_str(&new_conn_cid, conn->scid_buf, sizeof(conn->scid_buf)),
            xqc_sr_token_str(new_conn_cid.sr_token, conn->log_buf, sizeof(conn->log_buf)));
    ...
    if (path == NULL) {
        if (conn->conn_type == XQC_CONN_TYPE_SERVER) {
            ...
            xqc_log(conn->log, XQC_LOG_ERROR, 
                    ...
                    xqc_dcid_str(&packet_in->pi_pkt.pkt_dcid, conn->dcid_buf, sizeof(conn->dcid_buf)),
    ...
    if (conn->enable_multipath) {
        ...
        if (path == NULL) {
            xqc_log(conn->log, XQC_LOG_ERROR, 
                    ...
                    xqc_scid_str(&packet_in->pi_pkt.pkt_dcid, conn->dcid_buf, sizeof(conn->dcid_buf)),
                    packet_in->pi_path_id);
            return XQC_OK;
        }

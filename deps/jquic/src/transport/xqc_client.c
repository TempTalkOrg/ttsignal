...
xqc_connection_t *
jqc_client_connect(xqc_engine_t *engine, const xqc_conn_settings_t *conn_settings,
    ...
    const char *alpn, const struct sockaddr *peer_addr, socklen_t peer_addrlen, 
    jqc_timer_callback_t* timer_cbs, void *user_data)
{
    ...
    if (xqc_generate_cid(engine, NULL, &scid, 0, user_data) != XQC_OK
        || xqc_generate_cid(engine, NULL, &dcid, 0, user_data) != XQC_OK)
    {
        xqc_log(engine->log, XQC_LOG_ERROR,
    ...
    xqc_connection_t *xc = jqc_client_create_connection(
        engine, dcid, scid, conn_settings, server_host, no_crypto_flag,
        conn_ssl_config, alpn, timer_cbs, user_data);
    if (xc == NULL) {
        xqc_log(engine->log, XQC_LOG_ERROR,
    ...
    jqc_conn_main_logic_internal(xc);
...
const xqc_cid_t *
jqc_connect(xqc_engine_t *engine, const xqc_conn_settings_t *conn_settings, 
    const unsigned char *token, unsigned token_len, const char *server_host, 
    int no_crypto_flag, const xqc_conn_ssl_config_t *conn_ssl_config, 
    ...
    jqc_timer_callback_t* timer_cbs, void *user_data)
{
    ...
    conn = jqc_client_connect(engine, conn_settings, token, token_len, 
        server_host, no_crypto_flag, conn_ssl_config, alpn, peer_addr, 
...
xqc_connection_t *
jqc_client_create_connection(xqc_engine_t *engine, xqc_cid_t dcid, xqc_cid_t scid,
    ...
    jqc_timer_callback_t* timer_cbs, void *user_data)
{
    ...
    xqc_connection_t *xc = xqc_conn_create(engine, &dcid, &scid, settings, 
        timer_cbs, user_data, XQC_CONN_TYPE_CLIENT);
    if (xc == NULL) {
        return NULL;
    }

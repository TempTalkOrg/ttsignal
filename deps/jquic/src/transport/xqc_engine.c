...
xqc_engine_conns_hash_find(xqc_engine_t *engine, const xqc_cid_t *cid, char type)
{
    ...
    if (type == 's') {
        ...
        conn = xqc_str_hash_find(engine->conns_hash, hash, str);

    } else {
        /* search by peer's cid */
        conn = xqc_str_hash_find(engine->conns_hash_dcid, hash, str);
    }
    ...
    void *user_data)
{
    ...
    engine->lock = engine_callback->lock_callbacks.lock_create();
    if (engine->lock == NULL) {
        goto fail;
    }
...
xqc_engine_destroy(xqc_engine_t *engine)
{
    ...
    if (engine->lock) {
        engine->eng_callback.lock_callbacks.lock_destroy(engine->lock);
    }
...
xqc_engine_process_conn(xqc_connection_t *conn, xqc_usec_t now)
{
    ...
    xqc_log(conn->log, XQC_LOG_DEBUG, "|conn:%p|state:%s|flag:%s|now:%ui|",
            conn, xqc_conn_state_2_str(conn->conn_state), 
            xqc_conn_flag_2_str(conn->conn_flag, conn->log_buf, sizeof(conn->log_buf)), now);
...
void
jqc_conn_process(xqc_connection_t *conn, xqc_usec_t now)
{
    xqc_log(conn->log, XQC_LOG_DEBUG, "|conn:%p|state:%s|flag:%s|now:%ui|",
            conn, xqc_conn_state_2_str(conn->conn_state), 
            xqc_conn_flag_2_str(conn->conn_flag, conn->log_buf,
                sizeof(conn->log_buf)), now);
...
jqc_conn_main_logic(xqc_connection_t* conn)
{
    ...
    conn->conn_flag |= XQC_CONN_FLAG_RUNNING;

    xqc_log(conn->log, XQC_LOG_DEBUG, "|BEGIN|");

    xqc_usec_t now = xqc_monotonic_timestamp();
    xqc_engine_t* engine = conn->engine;

    jqc_conn_process(conn, now);
    ...
    if (XQC_UNLIKELY(conn->conn_state == XQC_CONN_STATE_CLOSED)) {
        ...
        if (xqc_engine_is_sendmmsg_on(engine)) {
            ...
            //xqc_log(engine->log, XQC_LOG_ERROR, "|===== call stack dump =====|"
            //        "conn:%p|%s", conn, xqc_conn_addr_str(conn));
        ...
        if (XQC_UNLIKELY(conn->conn_state == XQC_CONN_STATE_CLOSED)) {
            conn->conn_flag &= ~XQC_CONN_FLAG_TICKING;
            if (!(conn->conn_flag & XQC_CONN_FLAG_NO_DESTROY)) {
                xqc_log(conn->log, XQC_LOG_INFO, "|destroy conn from conns_active_pq after sending|"
                        "conn:%p|%s", conn, xqc_conn_addr_str(conn));
    ...
    /*
    * xqc_conn_process_conn may insert conns_active_pq, XQC_CONN_FLAG_TICKING prevents
    * duplicate insertions and must be placed after xqc_conn_process_conn.
    */
    conn->conn_flag &= ~XQC_CONN_FLAG_TICKING;
...
quit:
    ...
    xqc_log(conn->log, XQC_LOG_DEBUG, "|END|");
    return;
}
...
void jqc_conn_main_logic_internal(xqc_connection_t* conn) {
    ...
    conn->conn_flag |= XQC_CONN_FLAG_NO_DESTROY;
    ...
    xqc_connection_t **c)
{
    ...
    if (NULL == conn) {
        ...
        xqc_log(engine->log, XQC_LOG_DEBUG, "|can't find conn with sr|sr:%s",
                xqc_sr_token_str(sr_token, conn->log_buf, sizeof(conn->log_buf)));
        return -XQC_ERROR;
    }
    ...
    jqc_pre_process_t *out_result)
{
    ...
    xqc_memzero(out_result, sizeof(*out_result));

    /* reverse packet's dcid/scid to endpoint's scid/dcid */
    ret = xqc_packet_parse_cid(&out_result->scid, &out_result->dcid, engine->config->cid_len,
    ...
    out_result->conn = xqc_engine_conns_hash_find(engine, &out_result->scid, 's');

    /* can't find a connection by the cid from the packet */
    if (XQC_UNLIKELY(out_result->conn == NULL)) {
 ...
 */
xqc_connection_t*
jqc_engine_create_server_connection(xqc_engine_t *engine,
    const unsigned char *packet_in_buf, size_t packet_in_size,
    const struct sockaddr *local_addr, socklen_t local_addrlen,
    const struct sockaddr *peer_addr, socklen_t peer_addrlen,
    uint64_t path_id, xqc_usec_t recv_time, jqc_timer_callback_t* timer_cbs,
    void *user_data, xqc_cid_t *scid, xqc_cid_t *dcid, xqc_bool_t *is_sr_process,
    ...
    *is_sr_process = 0;
        ...
        if (XQC_PACKET_IS_LONG_HEADER(packet_in_buf)) {
                     ...
                     && (local_addr != NULL && peer_addr != NULL))
            {
                conn = xqc_conn_server_create(engine, local_addr, local_addrlen,
                                            ...
                                            conn_settings, timer_cbs, user_data);
                if (conn == NULL) {
    ...
    if (NULL == conn) {
        ...
        xqc_log(engine->log, lvl, "|fail to find connection, send reset|"
                ...
                packet_in_size, xqc_scid_str(scid, cid_buf, sizeof(cid_buf)), recv_time,
                xqc_peer_addr_str(peer_addr, peer_addrlen, peer_addr_str, sizeof(peer_addr_str)),
                xqc_local_addr_str(local_addr, local_addrlen, local_addr_str, sizeof(local_addr_str)));
...
/**
 * Pass received UDP packet payload into xquic connection.
 * @param recv_time   UDP packet received time in microsecond
 */
xqc_int_t
jqc_conn_packet_process(xqc_connection_t* conn,
    const unsigned char *packet_in_buf, size_t packet_in_size,
    const struct sockaddr *local_addr, socklen_t local_addrlen,
    const struct sockaddr *peer_addr, socklen_t peer_addrlen,
    ...
    xqc_cid_t* scid, xqc_cid_t* dcid, xqc_bool_t is_sr_process)
{
    xqc_int_t ret = 0;
    xqc_engine_t *engine = conn->engine;
    xqc_cid_t dcid_buf, scid_buf;   /* dcid: cid of peer; scid: cid of endpoint */
    ...
    if (!scid || !dcid) {
        xqc_cid_init_zero(&dcid_buf);
        xqc_cid_init_zero(&scid_buf);

        /* reverse packet's dcid/scid to endpoint's scid/dcid */
        ret = xqc_packet_parse_cid(&scid_buf, &dcid_buf, engine->config->cid_len,
                                   (unsigned char *)packet_in_buf, packet_in_size);
        if (XQC_UNLIKELY(ret != XQC_OK)) {
            xqc_log(engine->log, XQC_LOG_INFO, "|fail to parse cid|ret:%d|", ret);
            return -XQC_EILLPKT;

...
#include "common/xqc_log_event_callback.h"
...
#include "transport/xqc_transport_params.h"
...
typedef enum {
    ...
    XQC_CONN_FLAG_RUNNING_SHIFT,
    XQC_CONN_FLAG_NO_DESTROY_SHIFT,
...
typedef enum {
    ...
    XQC_CONN_FLAG_RUNNING               = 1ULL << XQC_CONN_FLAG_RUNNING_SHIFT,
    XQC_CONN_FLAG_NO_DESTROY            = 1ULL << XQC_CONN_FLAG_NO_DESTROY_SHIFT,
...
struct xqc_connection_s {
    ...
    // log buffer : conn_flag_buf needs 1024 bytes, frame_type_buf needs 128 bytes
    char                            log_buf[1024];
    char                            scid_buf[XQC_MAX_CID_LEN * 2 + 1];
    char                            dcid_buf[XQC_MAX_CID_LEN * 2 + 1];
};

const char *xqc_conn_flag_2_str(xqc_conn_flag_t conn_flag, char* out_buf, size_t out_buf_size);
...
xqc_connection_t *xqc_conn_create(xqc_engine_t *engine, xqc_cid_t *dcid, xqc_cid_t *scid,
    const xqc_conn_settings_t *settings, jqc_timer_callback_t* timer_cbs, void *user_data, 

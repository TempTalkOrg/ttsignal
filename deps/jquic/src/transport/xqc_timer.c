...
xqc_timer_ping_timeout(xqc_timer_type_t type, xqc_usec_t now, void *user_data)
{
    ...
    if (conn->conn_settings.ping_on/* && conn->conn_type == XQC_CONN_TYPE_CLIENT*/) {
        xqc_timer_set(&conn->conn_timer_manager, XQC_TIMER_PING, now, 
...
xqc_timer_unregister_gp_timer(xqc_timer_manager_t *manager, xqc_gp_timer_id_t gp_timer_id)
{
    ...
    xqc_list_for_each_safe(pos, next, &manager->gp_timer_list) {
        ...
        if (gp_timer->id == gp_timer_id) {
            xqc_timer_destroy_gp_timer(manager->timer_cbs, gp_timer);
            return XQC_OK;
        }
    }
    return XQC_ERROR;
...
void 
xqc_timer_destroy_gp_timer(jqc_timer_callback_t* timer_cbs, xqc_gp_timer_t *gp_timer)
{
    if (gp_timer->timer_id) {
        jqc_gp_timer_unset(timer_cbs, gp_timer, XQC_FALSE);
    }
...
void jqc_gp_timer_timeout_cb(void *user_data)
{
    xqc_usec_t now = xqc_realtime_timestamp();
    xqc_gp_timer_t *gp_timer = (xqc_gp_timer_t*)user_data;
    xqc_timer_manager_t* manager = gp_timer->manager;
    if (gp_timer->timeout_cb) {
...
void jqc_timer_timeout_cb(void *user_data)
{
    xqc_usec_t now = xqc_realtime_timestamp();
    xqc_timer_t *timer = (xqc_timer_t*)user_data;
    xqc_timer_manager_t* manager = timer->manager;
    xqc_timer_type_t type = timer->type;
    xqc_connection_t* conn = NULL;
    if (timer->timeout_cb) {
        timer->timeout_cb(timer->type, now, timer->user_data);
    ...
    switch (type)
    {
    case XQC_TIMER_ACK_INIT:
    case XQC_TIMER_ACK_HSK:
    case XQC_TIMER_ACK_01RTT:
    case XQC_TIMER_LOSS_DETECTION:
    case XQC_TIMER_PACING:
    case XQC_TIMER_NAT_REBINDING:
    case XQC_TIMER_PATH_IDLE:
    ...
    case XQC_TIMER_CONN_DRAINING:
    case XQC_TIMER_STREAM_CLOSE:
    case XQC_TIMER_PING:
    case XQC_TIMER_RETIRE_CID:
    case XQC_TIMER_LINGER_CLOSE:
    case XQC_TIMER_KEY_UPDATE:
    case XQC_TIMER_PMTUD_PROBING:
        conn = timer->user_data;
        break;
    default:
        break;
    }
    if (conn == NULL) {
        xqc_log(manager->log, XQC_LOG_ERROR, "|timer timeout cb error|type:%s|conn is null|", xqc_timer_type_2_str(type));
        return;
    }
    jqc_conn_main_logic(conn);
}

int32_t jqc_timer_set(jqc_timer_callback_t* timer_cbs, xqc_timer_t *timer, xqc_usec_t expire_time)
{
    if (timer_cbs && timer_cbs->timer_set && timer && timer_cbs->timer_update){
        if (timer->timer_id > 0) {
            timer_cbs->timer_update(timer_cbs->timer_ctx, timer->timer_id,
                expire_time, jqc_timer_timeout_cb, timer);
        } else {
            timer->timer_id = timer_cbs->timer_set(timer_cbs->timer_ctx, 
                expire_time, jqc_timer_timeout_cb, timer);
        }
        timer->timer_is_set = 1;
...
void jqc_timer_update(jqc_timer_callback_t* timer_cbs, xqc_timer_t *timer, xqc_usec_t expire_time)
{
    if (timer_cbs && timer_cbs->timer_update && timer){
        timer_cbs->timer_update(timer_cbs->timer_ctx, timer->timer_id,
            expire_time, jqc_timer_timeout_cb, timer);
        timer->expire_time = expire_time;
    }
...
void jqc_timer_unset(jqc_timer_callback_t* timer_cbs, xqc_timer_t *timer, xqc_bool_t only_cancel)
{
    if (timer_cbs && timer_cbs->timer_unset && timer && timer->timer_id > 0){
        timer_cbs->timer_unset(timer_cbs->timer_ctx, &timer->timer_id, only_cancel);
    }
    timer->timer_is_set = 0;
    timer->expire_time = 0;
}

int32_t jqc_gp_timer_set(jqc_timer_callback_t* timer_cbs, xqc_gp_timer_t *timer, xqc_usec_t expire_time)
{
    if (timer_cbs && timer_cbs->timer_set && timer_cbs->timer_update && timer){
        if (timer->timer_id > 0) {
            timer_cbs->timer_update(timer_cbs->timer_ctx, timer->timer_id,
                expire_time, jqc_gp_timer_timeout_cb, timer);
        } else {
            timer->timer_id = timer_cbs->timer_set(timer_cbs->timer_ctx,
                expire_time, jqc_gp_timer_timeout_cb, timer);
        }
...
void jqc_gp_timer_update(jqc_timer_callback_t* timer_cbs, xqc_gp_timer_t *timer, xqc_usec_t expire_time)
{
    if (timer_cbs && timer_cbs->timer_update != NULL){
        timer_cbs->timer_update(timer_cbs->timer_ctx, timer->timer_id,
            expire_time, jqc_gp_timer_timeout_cb, timer);
        timer->expire_time = expire_time;
    }
...
void jqc_gp_timer_unset(jqc_timer_callback_t* timer_cbs, xqc_gp_timer_t *timer, xqc_bool_t only_cancel)
{
    if (timer_cbs && timer_cbs->timer_unset && timer->timer_id > 0){
        timer_cbs->timer_unset(timer_cbs->timer_ctx, &timer->timer_id, only_cancel);
    }
    timer->timer_is_set = 0;
    timer->expire_time = 0;
...
void jqc_timer_next_tick(jqc_timer_callback_t *timer_cbs)
{
    if (timer_cbs && timer_cbs->timer_next_tick != NULL){
        timer_cbs->timer_next_tick(timer_cbs->timer_ctx);
    }
}

void jqc_timer_destroy_timers(xqc_timer_manager_t* manager)
{
    xqc_timer_t* timer;

    for (xqc_timer_type_t type = 0; type < XQC_TIMER_N; ++type) {
        timer = &manager->timer[type];
        jqc_timer_unset(manager->timer_cbs, timer, XQC_FALSE);
    }
}

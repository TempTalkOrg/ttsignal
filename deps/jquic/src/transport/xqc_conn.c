...
#include "tls/xqc_tls.h"
#include "transport/xqc_datagram.h"
#include <inttypes.h>
...
xqc_conn_settings_t default_conn_settings = {
    ...
    .ping_time_out              = XQC_PING_TIMEOUT,
    .enable_multipath           = 0,
...
xqc_server_set_conn_settings(const xqc_conn_settings_t *settings)
{
    ...
    if (settings->ping_time_out > 0) {
        default_conn_settings.ping_time_out = settings->ping_time_out;
    }
...
const char *
xqc_conn_flag_2_str(xqc_conn_flag_t conn_flag, char* out_buf, size_t out_buf_size)
{
    out_buf[0] = '\0';
    size_t pos = 0;
    int wsize;
    ...
    for (int i = 0; i < XQC_CONN_FLAG_SHIFT_NUM; i++) {
        if (conn_flag & 1ULL << i) {
            wsize = snprintf(out_buf + pos, out_buf_size - pos, "%s ", 
                             xqc_conn_flag_to_str[i]);
            if (wsize < 0 || wsize >= out_buf_size - pos) {
                break;
            }
            pos += wsize;
        }
...
xqc_conn_init_timer_manager(xqc_connection_t *conn)
{
    ...
    xqc_timer_init(timer_manager, conn->log, conn, &conn->timer_cbs);
    ...
    {
        xqc_timer_set(timer_manager, XQC_TIMER_PING, now, 
            /*XQC_PING_TIMEOUT * 1000*/conn->conn_settings.ping_time_out * 1000);
    }
...
xqc_conn_create(xqc_engine_t *engine, xqc_cid_t *dcid, xqc_cid_t *scid,
    const xqc_conn_settings_t *settings, jqc_timer_callback_t* timer_cbs, 
    void *user_data, xqc_conn_type_t type)
{
    xqc_int_t ret;
    ...
    if (xc->conn_settings.ping_time_out == 0) {
        xc->conn_settings.ping_time_out = XQC_PING_TIMEOUT;
    }
    ...
    JQC_LOCK_ENGINE(xc->engine);
    ret = xqc_insert_conns_hash(engine->conns_hash, xc,
        xc->scid_set.user_scid.cid_buf,
        xc->scid_set.user_scid.cid_len);
    JQC_UNLOCK_ENGINE(xc->engine);
    if (ret)
    {
        goto fail;
    }
    ...
    xqc_log(xc->log, XQC_LOG_DEBUG, "|success|scid:%s|dcid:%s|conn:%p|",
            xqc_scid_str(&xc->scid_set.user_scid, scid_buf, sizeof(scid_buf)), 
            xqc_dcid_str(&xc->dcid_set.current_dcid, dcid_buf, sizeof(dcid_buf)), xc);
...
xqc_conn_server_create(xqc_engine_t *engine, const struct sockaddr *local_addr,
    ...
    xqc_cid_t *dcid, xqc_cid_t *scid, xqc_conn_settings_t *settings, 
    jqc_timer_callback_t* timer_cbs, void *user_data)
{
    xqc_int_t           ret;
    xqc_connection_t   *conn;
    xqc_cid_t           new_scid;
    char                cid_buf[XQC_MAX_CID_LEN * 2 + 1];
    ...
    conn = xqc_conn_create(engine, dcid, &new_scid, settings, timer_cbs, 
        user_data, XQC_CONN_TYPE_SERVER);
    if (conn == NULL) {
    ...
    if (xqc_cid_in_cid_set(&conn->scid_set.cid_set, &conn->original_dcid) == NULL) {
         ...
         */
        JQC_LOCK_ENGINE(conn->engine);
        ret = xqc_insert_conns_hash(engine->conns_hash, conn,
            conn->original_dcid.cid_buf,
            conn->original_dcid.cid_len);
        JQC_UNLOCK_ENGINE(conn->engine);
        if (ret)
        {
            goto fail;
        }
        ...
        xqc_log(conn->log, XQC_LOG_INFO, "|hash odcid conn|odcid:%s|conn:%p|",
                xqc_dcid_str(&conn->original_dcid, cid_buf, sizeof(cid_buf)), conn);
    }
...
xqc_conn_destroy(xqc_connection_t *xc)
{
    ...
    /* destroy timers */
    jqc_timer_destroy_timers(&xc->conn_timer_manager);
...
void *
xqc_conn_get_transport_user_data(xqc_connection_t *conn)
{
...
void *
xqc_conn_get_alp_user_data(xqc_connection_t *conn)
{
    return conn->proto_data;
}
...
xqc_conn_send_ping(xqc_engine_t *engine, const xqc_cid_t *cid, void *ping_user_data)
{
    ...
    if (!conn) {
        ...
        xqc_log(engine->log, XQC_LOG_ERROR, "|can not find connection|cid:%s",
                xqc_scid_str(cid, cid_buf, sizeof(cid_buf)));
    ...
    jqc_conn_main_logic_internal(conn);
    return XQC_OK;
}
    ...
    xqc_packet_out_t *packet_out, xqc_send_type_t send_type)
{
    ...
    xqc_log(conn->log, XQC_LOG_DEBUG,
            ...
            xqc_frame_type_2_str(packet_out->po_frame_types, 
                conn->log_buf, sizeof(conn->log_buf)));
        ...
        if (sent != len) {
            ...
            if (sent == XQC_SOCKET_ERROR) {
                ...
                if (xqc_conn_should_close(conn, path)) {
                    ...
                    XQC_CONN_CLOSE_MSG(conn, "write socket error");
                    conn->conn_state = XQC_CONN_STATE_CLOSED;
                }
            }
            return -XQC_ESOCKET;
...
xqc_send_packet_with_pn(xqc_connection_t *conn, xqc_path_ctx_t *path, xqc_packet_out_t *packet_out)
{
    ...
    if (sent != conn->enc_pkt_len) {
        xqc_log(conn->log, XQC_LOG_ERROR,
                ...
                conn, path->path_id, packet_out->po_pkt.pkt_num, packet_out->po_used_size, sent,
                xqc_pkt_type_2_str(packet_out->po_pkt.pkt_type),
                xqc_frame_type_2_str(packet_out->po_frame_types, conn->log_buf, sizeof(conn->log_buf)), now);
        return sent;
        ...
        xqc_log(conn->log, XQC_LOG_INFO,
                ...
                xqc_pkt_type_2_str(packet_out->po_pkt.pkt_type),
                xqc_frame_type_2_str(packet_out->po_frame_types, conn->log_buf, sizeof(conn->log_buf)),
                path->path_send_ctl->ctl_bytes_in_flight, now, packet_out->po_stream_id, packet_out->po_stream_offset);
        xqc_log_event(conn->log, TRA_PACKET_SENT, packet_out);
    }
    ...
    xqc_packet_out_t *packet_out)
{
    ...
    xqc_log(c->log, XQC_LOG_DEBUG, "|conn:%p|path:%ui|pkt_num:%ui"
            ...
            packet_out->po_used_size,
            xqc_pkt_type_2_str(packet_out->po_pkt.pkt_type),
            xqc_frame_type_2_str(packet_out->po_frame_types, c->log_buf, sizeof(c->log_buf)),
...
xqc_conn_close(xqc_engine_t *engine, const xqc_cid_t *cid)
{
    ...
    xqc_log(conn->log, XQC_LOG_INFO, "|conn:%p|state:%s|flag:%s|", conn,
            ...
            xqc_conn_flag_2_str(conn->conn_flag, conn->log_buf, sizeof(conn->log_buf)));
...
xqc_int_t
jqc_conn_close(xqc_connection_t *conn_to_close, const xqc_cid_t *cid)
{
    xqc_int_t ret;
    xqc_connection_t *conn;
    xqc_engine_t *engine = conn_to_close->engine;
    ...
    if (conn != conn_to_close) {
        xqc_log(engine->log, XQC_LOG_ERROR, 
                "|conn_to_close is not the same as conn|conn_to_close:%p|conn:%p|", 
                conn_to_close, conn);
        conn = conn_to_close;
    }
...
xqc_conn_get_lastest_rtt(xqc_engine_t *engine, const xqc_cid_t *cid)
{
    ...
    if (!path) {
        xqc_log(engine->log, XQC_LOG_ERROR, "|can not find initial path|cid:%s",
                xqc_scid_str(cid, conn->scid_buf, sizeof(conn->scid_buf)));
        return 0;
    }
...
xqc_local_addr_str(
    ...
    size_t out_buf_size)
{
    if (!out_buf || out_buf_size < INET6_ADDRSTRLEN) {
        return NULL;
    }
    ...
    if (sa_local->sin_family == AF_INET) {
        if (inet_ntop(sa_local->sin_family, &sa_local->sin_addr, out_buf, local_addrlen) == NULL) {
            out_buf[0] = '\0';
        }

    } else {
        if (inet_ntop(sa_local->sin_family, &((struct sockaddr_in6*)sa_local)->sin6_addr,
                      out_buf, local_addrlen) == NULL)
        {
            out_buf[0] = '\0';
        }
    }
    ...
    size_t out_buf_size)
{
    ...
    if (sa_peer->sin_family == AF_INET) {
        if (inet_ntop(sa_peer->sin_family, &sa_peer->sin_addr, out_buf, peer_addrlen) == NULL) {
            out_buf[0] = '\0';
        }

    } else {
        if (inet_ntop(sa_peer->sin_family, &((struct sockaddr_in6*)sa_peer)->sin6_addr,
                      out_buf, peer_addrlen) == NULL)
        {
            out_buf[0] = '\0';
        }
    }
...
xqc_conn_addr_str(xqc_connection_t *conn)
{
    ...
    if (conn->addr_str_len == 0) {
        ...
        conn->addr_str_len = snprintf(conn->addr_str, sizeof(conn->addr_str), "l-%s-%d-%s p-%s-%d-%s",
                                      xqc_local_addr_str((struct sockaddr*)sa_local, conn->local_addrlen, local_addr_str, sizeof(local_addr_str)),
                                      ntohs(sa_local->sin_port), 
                                      xqc_scid_str(&conn->scid_set.user_scid, conn->scid_buf, sizeof(conn->scid_buf)),
                                      xqc_peer_addr_str((struct sockaddr*)sa_peer, conn->peer_addrlen, peer_addr_str, sizeof(peer_addr_str)),
                                      ntohs(sa_peer->sin_port), 
                                      xqc_dcid_str(&conn->dcid_set.current_dcid, conn->dcid_buf, sizeof(conn->dcid_buf)));
    }
...
xqc_path_addr_str(xqc_path_ctx_t *path)
{
    ...
    if (path->addr_str_len == 0) {
        ...
        path->addr_str_len = snprintf(path->addr_str, sizeof(path->addr_str), "l-%s-%d-%s p-%s-%d-%s",
                                      ...
                                      ntohs(sa_local->sin_port), 
                                      xqc_scid_str(&path->path_scid, scid_buf, sizeof(scid_buf)),
                                      xqc_peer_addr_str((struct sockaddr*)sa_peer, path->peer_addrlen, 
                                          peer_addr_str, sizeof(peer_addr_str)),
                                      ntohs(sa_peer->sin_port), 
                                      xqc_dcid_str(&path->path_dcid, dcid_buf, sizeof(dcid_buf)));
    }
    ...
    if (!(c->conn_flag & XQC_CONN_FLAG_DCID_OK)) {
        ...
        if (XQC_OK != xqc_cid_is_equal(&c->dcid_set.current_dcid, &pkt->pkt_scid)) {
            xqc_log(c->log, XQC_LOG_INFO, "|dcid change|ori:%s|new:%s|", 
                    xqc_dcid_str(&c->dcid_set.current_dcid, c->dcid_buf, sizeof(c->dcid_buf)),
                    xqc_scid_str(&pkt->pkt_scid, c->scid_buf, sizeof(c->scid_buf)));
            // TODO: DCID changes
        ...
        JQC_LOCK_ENGINE(c->engine);
        ret = xqc_insert_conns_hash(c->engine->conns_hash_dcid, c,
            c->dcid_set.current_dcid.cid_buf,
            c->dcid_set.current_dcid.cid_len);
        JQC_UNLOCK_ENGINE(c->engine);
        if (ret)
        {
...
xqc_conn_on_pkt_processed(xqc_connection_t *c, xqc_packet_in_t *pi, xqc_usec_t now)
{
    ...
    xqc_log(c->log, XQC_LOG_INFO, "|====>|conn:%p|path:%ui|size:%uz|pkt_type:%s|pkt_num:%ui|frame:%s|recv_time:%ui|",
            ...
            xqc_frame_type_2_str(pi->pi_frame_types, c->log_buf, sizeof(c->log_buf)), pi->pkt_recv_time);
    return ret;
}
...
#endif
{
    ...
    if (path == NULL) {
        xqc_log(conn->log, XQC_LOG_INFO, "|ignore unknown path|scid:%s|", 
            xqc_scid_str(scid, conn->scid_buf, sizeof(conn->scid_buf)));
        return;
    }
    ...
    if (scid->state == XQC_CID_UNUSED) {
        ...
        if (ret < 0) {
            xqc_log(conn->log, XQC_LOG_ERROR, "|xqc_cid_switch_to_next_state error|scid:%s|",
                    xqc_scid_str(&scid->cid, conn->scid_buf, sizeof(conn->scid_buf)));
            return ret;
        }
    }
...
xqc_conn_set_cid_retired_ts(xqc_connection_t *conn, xqc_cid_inner_t *inner_cid)
{
    ...
    xqc_log(conn->log, XQC_LOG_DEBUG, 
            ...
            xqc_scid_str(&inner_cid->cid, conn->scid_buf, sizeof(conn->scid_buf)),
...
xqc_conn_reassemble_packet(xqc_connection_t *conn, xqc_packet_out_t *ori_po)
{
    ...
    xqc_log(conn->log, XQC_LOG_DEBUG,
            ...
            xqc_frame_type_2_str(new_po->po_frame_types, conn->log_buf, sizeof(conn->log_buf)));
...
xqc_conn_on_recv_retry(xqc_connection_t *conn, xqc_cid_t *retry_scid)
{
    ...
    if (ret != XQC_OK) {
        xqc_log(conn->log, XQC_LOG_ERROR,
                ...
                xqc_scid_str(retry_scid, conn->scid_buf, sizeof(conn->scid_buf)), ret);
        return ret;
    }
...
xqc_conn_get_local_transport_params(xqc_connection_t *conn, xqc_transport_params_t *params)
{
        ...
        && conn->original_dcid.cid_len > 0)
    {
        ...
        xqc_log(conn->log, XQC_LOG_INFO, "|generate sr_token[%s] for cid[%s]",
                xqc_sr_token_str(params->stateless_reset_token, conn->log_buf, sizeof(conn->log_buf)),
                xqc_scid_str(&conn->original_dcid, conn->scid_buf, sizeof(conn->scid_buf)));
...
xqc_conn_tls_transport_params_cb(const uint8_t *tp, size_t len, void *user_data)
{
    ...
    xqc_engine_t* engine = (xqc_engine_t*)conn->engine;
    ...
    if (params.stateless_reset_token_present) {
        ...
        if (NULL != node) {
            ...
            xqc_log(conn->log, XQC_LOG_INFO, "|store sr_token with cid: %s"
                    "|token:%s", xqc_dcid_str(&cid_node->cid, conn->scid_buf, sizeof(conn->scid_buf)),
                    xqc_sr_token_str(params.stateless_reset_token, conn->log_buf, sizeof(conn->log_buf)));

            JQC_LOCK_ENGINE(engine);
            ret = xqc_insert_conns_hash(conn->engine->conns_hash_sr_token, conn,
                cid_node->cid.sr_token,
                XQC_STATELESS_RESET_TOKENLEN);
            JQC_UNLOCK_ENGINE(engine);
            if (ret)
            {
    ...
    xqc_list_for_each_safe(pos, next, &conn->dcid_set.cid_set.list_head) {
        ...
        if (0 == res) {
            xqc_log(conn->log, XQC_LOG_INFO, "|====>|receive stateless reset"
                    "|cid:%s", xqc_dcid_str(&cid->cid, conn->dcid_buf, 
                        sizeof(conn->dcid_buf)));
...
const char* jqc_conn_close_msg(xqc_connection_t* conn)
{
    return conn->conn_close_msg?conn->conn_close_msg:"unknown close reason";
}

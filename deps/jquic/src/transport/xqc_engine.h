...
typedef struct xqc_engine_s {
    ...
    /* h3 context per engine*/
    xqc_h3_ctx_t                   *h3_ctx; 

} xqc_engine_t;


#define JQC_LOCK_ENGINE(x)  (x)->eng_callback.lock_callbacks.lock_acquire((x)->lock)
#define JQC_UNLOCK_ENGINE(x)  (x)->eng_callback.lock_callbacks.lock_release((x)->lock)
...
void jqc_conn_process(xqc_connection_t *conn, xqc_usec_t now);

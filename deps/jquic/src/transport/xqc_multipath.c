...
#include "http3/xqc_h3_conn.h" /* TODO:delete me */
...
xqc_path_create(xqc_connection_t *conn, xqc_cid_t *scid, xqc_cid_t *dcid)
{
    ...
    if (scid == NULL) {
        ...
        if (inner_cid == NULL) {
            xqc_log(conn->log, XQC_LOG_DEBUG, "|invalid scid:%s|", 
    ...
    xqc_log(conn->engine->log, XQC_LOG_DEBUG, "|path:%ui|dcid:%s|scid:%s|create_path_count:%ud|",
            path->path_id, 
            xqc_dcid_str(&path->path_dcid, conn->dcid_buf, sizeof(conn->dcid_buf)), 
            xqc_scid_str(&path->path_scid, conn->scid_buf, sizeof(conn->scid_buf)),
            conn->create_path_count);
        ...
        xqc_list_for_each_safe(pos, next, &conn->conn_paths_list) {
            ...
            if (xqc_is_same_addr(peer_addr, (struct sockaddr *)active_path->peer_addr)) {
                ...
                xqc_log(conn->engine->log, XQC_LOG_STATS, "|MP|path:%ui|conn:%s|cannot activate this path, due to the same IP|curIP:%s|conflictIP:%s|",
                        ...
                        xqc_local_addr_str((struct sockaddr*)active_path->peer_addr, 
                            active_path->peer_addrlen, local_addr_str, sizeof(local_addr_str)));
                return XQC_OK;
            }
        }

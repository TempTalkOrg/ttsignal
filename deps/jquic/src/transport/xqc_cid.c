...
unsigned char *
xqc_dcid_str(const xqc_cid_t *dcid, unsigned char *out_buf, size_t out_buf_size)
{
    if (!out_buf || out_buf_size < dcid->cid_len * 2 + 1) {
        return NULL;
    }
    xqc_hex_dump(out_buf, dcid->cid_buf, dcid->cid_len);
    out_buf[dcid->cid_len * 2] = '\0';
    return out_buf;
}

unsigned char *
xqc_scid_str(const xqc_cid_t *scid, unsigned char *out_buf, size_t out_buf_size)
{
    if (!out_buf || out_buf_size < scid->cid_len * 2 + 1) {
        return NULL;
    }
    xqc_hex_dump(out_buf, scid->cid_buf, scid->cid_len);
    out_buf[scid->cid_len * 2] = '\0';
    return out_buf;
}

char *
xqc_sr_token_str(const char *sr_token, char* out_buf, size_t out_buf_size)
{
    if (!out_buf || out_buf_size < XQC_STATELESS_RESET_TOKENLEN * 2 + 1) {
        return NULL;
    }
    xqc_hex_dump(out_buf, sr_token, XQC_STATELESS_RESET_TOKENLEN);
    out_buf[XQC_STATELESS_RESET_TOKENLEN * 2] = '\0';
    return out_buf;
}

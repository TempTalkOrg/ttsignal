    ...
    const xqc_cid_t *cid, xqc_stream_settings_t *settings, void *user_data)
{
    ...
    xqc_log(engine->log, XQC_LOG_DEBUG, "|success|stream_id:%ui|conn:%p|conn_state:%s|flag:%s|",
            ...
            xqc_conn_flag_2_str(h3_conn->conn->conn_flag, h3_conn->conn->log_buf, 
                sizeof(h3_conn->conn->log_buf)));
    return h3_request;
}
...
xqc_h3_request_init_callbacks(xqc_h3_request_t *h3r)
{
    ...
    xqc_int_t ret = xqc_h3_ctx_get_app_callbacks(h3r->h3_stream->h3c->conn->engine, &h3_cbs);
...
xqc_stream_id_t
xqc_h3_request_id(xqc_h3_request_t *h3_request)
{
    return h3_request->h3_stream->stream_id;
}

...
xqc_h3_stream_send_headers(xqc_h3_stream_t *h3s, xqc_http_headers_t *headers, uint8_t fin)
{
    ...
    xqc_log(h3c->log, XQC_LOG_DEBUG, "|write:%z|stream_id:%ui|fin:%ud|conn:%p|flag:%s|", write,
            h3s->stream_id, (unsigned int)fin, h3c->conn, 
            xqc_conn_flag_2_str(h3c->conn->conn_flag, h3c->conn->log_buf, 
                sizeof(h3c->conn->log_buf)));

    h3s->flags &= ~XQC_HTTP3_STREAM_NEED_WRITE_NOTIFY;

    jqc_conn_main_logic_internal(h3c->conn);
...
xqc_h3_stream_send_data(xqc_h3_stream_t *h3s, unsigned char *data, size_t data_size, uint8_t fin)
{
    ...
    jqc_conn_main_logic_internal(h3s->h3c->conn);
...
xqc_h3_stream_create_notify(xqc_stream_t *stream, void *user_data)
{
    ...
    {
        if (h3c->local_h3_conn_settings.extended_connect == XQC_TRUE
            && h3c->local_h3_conn_settings.datagram == XQC_TRUE
            && conn->app_proto_cbs.wt_stream_cbs.stream_create_notify != NULL) {
            stream->stream_if = &conn->app_proto_cbs.wt_stream_cbs;
            return stream->stream_if->stream_create_notify(stream, user_data);
        } else {

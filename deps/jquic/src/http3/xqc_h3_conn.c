    ...
    socklen_t peer_addrlen, jqc_timer_callback_t* timer_cbs, void *user_data)
{
    ...
    conn = jqc_client_connect(engine, conn_settings, token, token_len, server_host, no_crypto_flag, 
                              conn_ssl_config, xqc_h3_alpn[conn_settings->proto_version], peer_addr,
                              peer_addrlen, timer_cbs, user_data);
    if (!conn) {
...
xqc_h3_conn_set_settings(xqc_h3_conn_t *h3_conn, const xqc_h3_conn_settings_t *h3_conn_settings)
{
    ...
    if (h3_conn_settings->extended_connect) {
        settings->extended_connect = h3_conn_settings->extended_connect;
    }

    if (h3_conn_settings->datagram) {
        settings->datagram = h3_conn_settings->datagram;
    }
...
xqc_h3_conn_init_callbacks(xqc_h3_conn_t *h3c)
{
    ...
    xqc_int_t ret = xqc_h3_ctx_get_app_callbacks(h3c->conn->engine, &h3_cbs);
...
xqc_h3_conn_on_settings_entry_received(uint64_t identifier, uint64_t value, void *user_data)
{
    ...
    case XQC_H3_SETTINGS_EXTENDED_CONNECT:
        h3c->peer_h3_conn_settings.extended_connect = value;
        break;

        /* HTTP Datagrams, RFC 9297 */
    case XQC_H3_SETTINGS_DATAGRAM:
        h3c->peer_h3_conn_settings.datagram = value;
        break;
...
void
xqc_h3_conn_packet_acked_notify(xqc_connection_t *conn, xqc_usec_t ack_delay_time,
    size_t acked_bytes, size_t inflight_bytes, xqc_stream_id_t stream_id, 
    void* conn_user_data, void* conn_proto_data)
{
    xqc_h3_conn_t *h3c = (xqc_h3_conn_t*)conn_proto_data;

    if (h3c->h3_conn_callbacks.h3_conn_packet_acked) {
        xqc_log(conn->log, XQC_LOG_DEBUG, "|Packet acked notify|");

        h3c->h3_conn_callbacks.h3_conn_packet_acked(h3c, ack_delay_time,
                                                    acked_bytes, inflight_bytes,
                                                    stream_id, h3c->user_data);
    }
}
...
const xqc_conn_callbacks_t h3_conn_callbacks = {
    ...
    .conn_packet_acked          = xqc_h3_conn_packet_acked_notify,
};
    ...
    xqc_cid_t* scid, xqc_cid_t* dcid, xqc_bool_t is_sr_process)
{
    ...
    return jqc_conn_packet_process(conn, packet_in_buf, packet_in_size,
        local_addr, local_addrlen, peer_addr, peer_addrlen, path_id, recv_time,
        user_data, scid, dcid, is_sr_process);
}

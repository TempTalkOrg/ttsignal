...
xqc_h3_ctx_init(xqc_engine_t *engine, xqc_h3_callbacks_t *h3_cbs)
{
    ...
    if (NULL == engine->h3_ctx) {
        engine->h3_ctx = xqc_malloc(sizeof(xqc_h3_ctx_t));
        if (NULL == engine->h3_ctx) {
            return -XQC_EMALLOC;
        }
    }
...
xqc_int_t
xqc_h3_ctx_get_app_callbacks(xqc_engine_t *engine, xqc_h3_callbacks_t **h3_cbs)
{
    if (NULL == engine || NULL == engine->h3_ctx) {
        return -XQC_EFATAL;
    }

    *h3_cbs = &engine->h3_ctx->h3_cbs;
    return XQC_OK;
}

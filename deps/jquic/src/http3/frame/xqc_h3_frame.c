...
#include "http3/frame/xqc_h3_frame.h"
...
xqc_h3_frm_write_settings(xqc_list_head_t *send_buf, xqc_h3_conn_settings_t *setting, uint8_t fin)
{
    ...
    settings[count].identifier.vi = XQC_H3_SETTINGS_EXTENDED_CONNECT;
    settings[count].value.vi = setting->extended_connect;
    len += xqc_put_varint_len(settings[count].identifier.vi);
    len += xqc_put_varint_len(settings[count].value.vi);
    ++count;

    settings[count].identifier.vi = XQC_H3_SETTINGS_DATAGRAM;
    settings[count].value.vi = setting->datagram;
    len += xqc_put_varint_len(settings[count].identifier.vi);
    len += xqc_put_varint_len(settings[count].value.vi);
    ++count;

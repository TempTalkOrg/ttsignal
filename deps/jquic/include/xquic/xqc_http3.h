...
typedef void (*xqc_h3_conn_packet_ack_notify_pt)(xqc_h3_conn_t *h3_conn, xqc_usec_t ack_delay_time,
    size_t acked_bytes, size_t inflight_bytes, xqc_stream_id_t stream_id, void *h3c_user_data);
...
typedef struct xqc_h3_conn_settings_s {
    ...
    /**
     * @brief 是否开启 Extended CONNECT, RFC 9220
     */
    xqc_bool_t  extended_connect;

    /**
     * @brief 是否开启 HTTP Datagrams, RFC 9297
     */
    xqc_bool_t  datagram;
...
typedef struct xqc_h3_conn_callbacks_s {
    ...
    /* packet ack callback. which will be triggered when packet is acked */
    xqc_h3_conn_packet_ack_notify_pt    h3_conn_packet_acked;          /* optional */
...
const xqc_cid_t *xqc_h3_connect(xqc_engine_t *engine, const xqc_conn_settings_t *conn_settings,
    ...
    socklen_t peer_addrlen, jqc_timer_callback_t* timer_cbs, void *user_data);
...
/**
 * @brief get the close message of a h3 connection
 * 
 * @param h3_conn the h3 connection handle
 * @return the close message of the h3 connection
 */
XQC_EXPORT_PUBLIC_API
const char* jqc_h3_conn_close_msg(xqc_h3_conn_t* h3_conn);

/**
 * @brief the API to process a packet over the h3 connection
 * 
 * @param h3_conn the h3 connection handle
 * @param packet_in_buf the packet buffer
 * @param packet_in_size the packet size
 * @param local_addr the local address
 * @param local_addrlen the local address length
 * @param peer_addr the peer address
 * @param peer_addrlen the peer address length
 * @param path_id the path id
 * @param recv_time the receive time of the packet
 * @param user_data user data of the packet callback functions
 * @param scid the source connection id
 * @param dcid the destination connection id
 * @param is_sr_process the flag to indicate if the packet is processed as a server response
 * @return the result of processing the packet
 */
XQC_EXPORT_PUBLIC_API
xqc_int_t
jqc_h3_conn_packet_process(xqc_h3_conn_t* h3_conn,
...
/**
 * @brief the API to finish the receive of a packet over the h3 connection
 * 
 * @param h3_conn the h3 connection handle
 */
XQC_EXPORT_PUBLIC_API
void jqc_h3_conn_finish_recv(xqc_h3_conn_t* h3_conn);

/**
 * @brief get the stream id of a h3 request
 * 
 * @param h3_request the h3 request handle
 * @return the stream id of the h3 request
 */
XQC_EXPORT_PUBLIC_API
xqc_stream_id_t xqc_h3_request_id(xqc_h3_request_t *h3_request);

...
/* timer  callback */
typedef struct jqc_timer_callback_s {
    void* timer_ctx;
    int32_t(*timer_set)(void* ctx, xqc_usec_t expire_time, void(*timeout_cb)(void*), void* user_data);
    void (*timer_update)(void* ctx, int32_t timer_id, xqc_usec_t expire_time, void(*timeout_cb)(void*), void* user_data);
    void (*timer_unset)(void* ctx, int32_t *timer_id, xqc_bool_t only_cancel);
    void (*timer_next_tick)(void* ctx);
}jqc_timer_callback_t;
...
/**
 * @brief print stack callback
 *
 */
typedef void (*jqc_print_stack_pt)(const char *line, void *engine_user_data);
...
/**
 * @brief lock callback functions
 */
typedef struct jqc_lock_callbacks_s {
    /**
     * create lock callback function
     */
    void* (*lock_create)();

    /**
     * acquire lock callback function
     */
    void (*lock_acquire)(void *lock);

    /**
     * release lock callback function
     */
    void (*lock_release)(void *lock);

    /**
     * destroy lock callback function
     */
    void (*lock_destroy)(void *lock);

} jqc_lock_callbacks_t;

/**
 * @brief pre-process result
 */
typedef struct jqc_pre_process_s {
    /**
     * DCID
     */
    xqc_cid_t           dcid;
...
// add by antoniozhou begain
/** 
 * @brief Packet data acked callback function.
 *
 * if application send a APP_DATA frame with xqc_stream_send function, this callback function will be
 * triggered when this APP_DATA frame is acked by peer.
 */
typedef void (*xqc_conn_packet_ack_notify_pt)(xqc_connection_t *conn, xqc_usec_t ack_delay_time,
    size_t acked_bytes, size_t inflight_bytes, xqc_stream_id_t stream_id, void *conn_user_data, 
    void *conn_proto_data);
// add by antoniozhou end
...
typedef struct xqc_conn_callbacks_s {
    ...
    // add by antoniozhou begain
    /**
     * packet acked callback. OPTIONAL for both client and server
     */
    xqc_conn_packet_ack_notify_pt       conn_packet_acked;
    // add by antoniozhou end
...
typedef struct xqc_app_proto_callbacks_s {
    ...
    /* QUIC webtransport stream callback functions */
    xqc_stream_callbacks_t      wt_stream_cbs;
...
typedef struct xqc_engine_callback_s {
    ...
    /* lock utilities callback, REQUIRED */
    jqc_lock_callbacks_t            lock_callbacks;

    /* print stack callback, OPTIONAL */
    jqc_print_stack_pt              print_stack_cb;
...
typedef struct xqc_conn_settings_s {
    ...
    xqc_msec_t                  ping_time_out;      /* ping timeout interval */
...
XQC_EXPORT_PUBLIC_API
xqc_connection_t* jqc_engine_create_server_connection(
    ...
    socklen_t peer_addrlen, uint64_t path_id, xqc_usec_t recv_time, 
    jqc_timer_callback_t* timer_cbs, void *user_data, xqc_cid_t* scid, 
    xqc_cid_t* dcid, xqc_bool_t *is_sr_process, xqc_conn_settings_t* conn_settings);

XQC_EXPORT_PUBLIC_API
xqc_int_t jqc_conn_packet_process(xqc_connection_t* conn,
    const unsigned char *packet_in_buf, size_t packet_in_size,
    const struct sockaddr *local_addr, socklen_t local_addrlen,
    const struct sockaddr *peer_addr, socklen_t peer_addrlen,
    uint64_t path_id, xqc_usec_t recv_time, void *user_data,
    xqc_cid_t* scid, xqc_cid_t* dcid, xqc_bool_t is_sr_process);

XQC_EXPORT_PUBLIC_API
xqc_int_t jqc_engine_packet_pre_process(xqc_engine_t* engine,
    const unsigned char *packet_in_buf, size_t packet_in_size,
    jqc_pre_process_t *out_result);


/**
 * @brief Process single connection, application implements MUST call this function in timer callback
 */
XQC_EXPORT_PUBLIC_API
void jqc_conn_main_logic(xqc_connection_t *conn);


/**
 * user should call after a number of packet processed in jqc_conn_packet_process
 * call after recv a batch packets, may destroy connection when error
 */
XQC_EXPORT_PUBLIC_API
void jqc_conn_finish_recv(xqc_connection_t* conn);


/*
 * @brief Get conn close message
 */
XQC_EXPORT_PUBLIC_API
 ...
 */
XQC_EXPORT_PUBLIC_API
const xqc_cid_t *jqc_connect(xqc_engine_t *engine,
    ...
    const struct sockaddr *peer_addr, socklen_t peer_addrlen,
    const char *alpn, jqc_timer_callback_t* timer_cbs, void *user_data);
...
/**
 * Get transport user data
 */
XQC_EXPORT_PUBLIC_API
void *xqc_conn_get_transport_user_data(xqc_connection_t *conn);
 ...
 */
XQC_EXPORT_PUBLIC_API
void *xqc_conn_get_alp_user_data(xqc_connection_t *conn);
 ...
 */
XQC_EXPORT_PUBLIC_API
unsigned char *xqc_scid_str(const xqc_cid_t *scid, unsigned char* out_buf, size_t out_buf_size);

XQC_EXPORT_PUBLIC_API
unsigned char *xqc_dcid_str(const xqc_cid_t *dcid, unsigned char* out_buf, size_t out_buf_size);
